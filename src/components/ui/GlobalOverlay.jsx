...
const ContentCard = ({ content, isOpen, onClose, isMobile }) => {
    ...
    const certificateItems = Array.isArray(content.items) ? content.items : [];
    const [certPage, setCertPage] = useState(0);
    const certItemsPerPage = 4;
    const certPageCount = Math.max(1, Math.ceil(certificateItems.length / certItemsPerPage));
    const visibleCertificateItems = certificateItems.slice(certPage * certItemsPerPage, certPage * certItemsPerPage + certItemsPerPage);
            ...
            >
                <div
                    style={{
                        ...
                        ...cardStyle,
                        // Override styles for grid layout or image viewer to be centered and wider
                        ...(content.layout === 'certificate_grid' || content.layout === 'image_viewer' ? {
                            // Make it centered and wide on desktop
                            width: isMobile ? '95vw' : 'clamp(300px, 80vw, 1000px)',
                            height: 'clamp(450px, 85vh, 850px)',
                            maxHeight: '85vh',
                            left: '50%',
                            top: '50%',
                            right: 'auto',
                ...
                >
                    ...
                    {/* === LAYOUT: IMAGE VIEWER (Full Certificate Display) === */}
                    {content.layout === 'image_viewer' ? (
                        <div style={{
                            position: 'relative',
                            flex: 1,
                            minHeight: 0,
                            ...
                            <div style={{
                                ...
                                <img
                                    src={content.image}
                                    alt={content.title || 'Certification'}
                                    style={{
                                        maxWidth: '100%',
                                        maxHeight: '100%',
                                        objectFit: 'contain',
                        ...
                        <div style={{ position: 'relative', flex: 1, minHeight: 0 }}>
                            <div
                                ...
                                style={{
                                    ...
                                    padding: isMobile ? '1rem 0.5rem 1rem 0.5rem' : '1rem 3.5rem 1rem 1.5rem',
                                    ...getStaggerStyle(200)
                                }}>
                                {visibleCertificateItems.map((item, index) => (
                                    <button
                                        ...
                                        onMouseOver={(e) => {
                                            ...
                                            e.currentTarget.style.boxShadow = '6px 8px 0px rgba(0,0,0,0.12)';
                                        }}
                                        onMouseOut={(e) => {
                                    ...
                                    >
                                        ...
                                        <div>
                                            <h4 style={{ margin: '0 0 0.4rem 0', fontSize: '1.1rem', fontWeight: 700, fontFamily: "'Rubik Scribble', cursive" }}>
                                                {item.label}
                                            </h4>
                                            <span style={{ fontSize: '1rem', color: '#4a4a4a', fontFamily: "'Cabin Sketch', cursive", fontWeight: 700 }}>
                                                {item.date}
                                            </span>
                                        </div>
                                    </button>
                            ...
                            {certificateItems.length > 0 && (
                                <>
                                    <button
                                        type="button"
                                        onClick={() => setCertPage((page) => Math.max(page - 1, 0))}
                                        disabled={certPage === 0}
                                        aria-label="Previous certificates"
                                        style={{
                                            position: 'absolute',
                                            left: isMobile ? '0.35rem' : '0.75rem',
                                            top: '50%',
                                            ...
                                            borderRadius: '50%',
                                            border: '2px solid #1a1a1a',
                                            backgroundColor: certPage === 0 ? '#e7e7e7' : '#f9f9f9',
                                            color: '#1a1a1a',
                                            fontSize: '1.5rem',
                                            display: 'flex',
                                            alignItems: 'center',
                                            justifyContent: 'center',
                                            cursor: certPage === 0 ? 'not-allowed' : 'pointer',
                                            boxShadow: '3px 3px 0 rgba(0,0,0,0.1)',
                                            padding: 0,
                                            opacity: certPage === 0 ? 0.6 : 1,
                                            zIndex: 2
                                        }}
                                    >
                                        ←
                                    ...
                                    <button
                                        type="button"
                                        onClick={() => setCertPage((page) => Math.min(page + 1, certPageCount - 1))}
                                        disabled={certPage >= certPageCount - 1}
                                        aria-label="Next certificates"
                                        style={{
                                            position: 'absolute',
                                            right: isMobile ? '0.35rem' : '0.75rem',
                                            top: '50%',
                                            transform: 'translateY(-50%)',
                                            width: isMobile ? '2.5rem' : '3rem',
                                            height: isMobile ? '2.5rem' : '3rem',
                                            borderRadius: '50%',
                                            border: '2px solid #1a1a1a',
                                            backgroundColor: certPage >= certPageCount - 1 ? '#e7e7e7' : '#f9f9f9',
                                            color: '#1a1a1a',
                                            fontSize: '1.5rem',
                                            display: 'flex',
                                            alignItems: 'center',
                                            justifyContent: 'center',
                                            cursor: certPage >= certPageCount - 1 ? 'not-allowed' : 'pointer',
                                            boxShadow: '3px 3px 0 rgba(0,0,0,0.1)',
                                            padding: 0,
                                            opacity: certPage >= certPageCount - 1 ? 0.6 : 1,
                                            zIndex: 2
                                        }}
                                    >
                                        →
