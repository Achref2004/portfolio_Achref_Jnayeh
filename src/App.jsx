...
if (isLowEnd) {
  const CORE_TEXTURES = [...TEXTURES_ENTREE, ...TEXTURES_COULOIR, ...TEXTURES_UI, ...ASSETS_IMAGES];
  const filteredCore = filterTexturesByDevice(CORE_TEXTURES, supportsHover);
  const filteredAbout = filterTexturesByDevice(TEXTURES_A_PROPOS, supportsHover);

  filteredCore.forEach(path => useTexture.preload(path));
  filteredAbout.forEach(path => useLoader.preload(TextureLoader, path));
} else {
  const filteredAll = filterTexturesByDevice(PRECHARGEMENT_COMPLET, supportsHover);
  const filteredLoader = filterTexturesByDevice(PRECHARGEMENT_CHARGEUR, supportsHover);
...
export default function App() {
  ...
  useEffect(() => {
    const filteredImages = filterTexturesByDevice(ASSETS_IMAGES, supportsHover);
